...
from base_model.base_blocks import DMFB, SpectralNorm, SelfAttention, _activation, get_pad
# import models.FineEdgeGAN.model.blocks as B
...
class FineBottleneck(nn.Module):
    def __init__(self, nf, res_blocks_num=8, dmfb_blocks_num=4, attention_mode="self"):
        ...
        for _ in range(res_blocks_num):
            block = B.ResConv(nf, kernel_size=3, dilation=2, padding=2)  # [192, 64, 64]
...
class InpaintingGenerator(nn.Module):
    def __init__(self, in_nc=4, out_nc=3, nf=48,
        ...
        blocks = []
        for _ in range(res_blocks_num):
            block = B.ResConv(4 * nf, kernel_size=3, dilation=2, padding=2)  # [192, 64, 64]
            blocks.append(block)

        self.coarse = nn.Sequential(*blocks)
        self.fine = FineBottleneck(4 * nf, res_blocks_num, dmfb_block_num, attention_mode="self")
        ...
        self.out = nn.Sequential(
            B.conv_block(nf//2, out_nc, 3, stride=1, padding=1, norm='none', activation='tanh')
        )
...
class InpaintingDiscriminator(nn.Module):
    def __init__(self, device, in_nc=3, kernel_size=4, nf=48):
        ...
        self.edge_dis = nn.Sequential(
            B.SobelFilter(device, in_nc=3, filter_c=1, padding=get_pad(256, 3, 1), stride=1),
